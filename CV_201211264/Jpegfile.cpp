...
// made by Jeong Ki Young 15 SEP 1998
BOOL JpegFile::RGBFromBGR(BYTE *buf, UINT widthPix, UINT height)
{
	if (buf==NULL)
		return FALSE;
	...
	for (row=0;row<height;row++) {
		for (col=0;col<widthPix;col++) {
			LPBYTE pRed, /*pGrn,*/ pBlu;
			pBlu = buf + row * widthPix * 3 + col * 3;
//			pGrn = buf + row * widthPix * 3 + col * 3 + 1;
			pRed = buf + row * widthPix * 3 + col * 3 + 2;
...
BYTE * JpegFile::OpenFileToRGB(FILE *infile,
							   UINT *width,
							   UINT *height)
...
BOOL JpegFile::RGBToOpenFile(FILE *outfile, 
							BYTE *dataBuf,
							UINT widthPix,
							UINT height,
							BOOL color, 
